...
| `evals/` | 6 tareas reales, brazo de control (`ARM=off`), historico y emisor a LangSmith (opt-in, no corre en `test/`, cuesta llamadas reales a `claude -p`) | medir si el harness **sirve**, no solo si el agente pasa — ver [`evals/README.md`](evals/README.md) |
